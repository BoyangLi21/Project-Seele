...
def main(folder):
 ...
 lines.append(f"file '{(folder/frames[-1]['file']).resolve().as_posix()}'");source=OUT/'native_first_battle.concat.txt';source.write_text('\n'.join(lines)+'\n',encoding='utf8')
 ...
 for time,name,gain in cues:
  at=min(active,key=lambda f:abs(f['scene_seconds']-time))['seconds'];path=ROOT/'artifacts/first_battle_world_r10/audio'/('eva_'+name+'.wav')
 ...
 audio=audio[:round(duration*RATE)];audio*=min(1,.88/max(abs(audio)));wav=OUT/'native_first_battle_review_mix.wav'
 ...
 target=OUT/'eva_sachiel_first_battle_r10.mp4'
