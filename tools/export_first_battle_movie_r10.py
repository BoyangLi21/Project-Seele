...
def main(folder,output=OUT,name='eva_sachiel_first_battle_r10',landing_time=11.7):
 ...
 subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-f','concat','-safe','0','-i',str(source),'-i',str(wav),'-fps_mode','vfr','-c:v','libx264','-threads','2','-preset','fast','-crf','22','-pix_fmt','yuv420p','-c:a','aac','-b:a','128k','-shortest','-movflags','+faststart',str(target)],check=True)
...
if __name__=='__main__':
 ap=argparse.ArgumentParser();ap.add_argument('folder',type=Path);ap.add_argument('--output-dir',type=Path,default=OUT);ap.add_argument('--name',default='eva_sachiel_first_battle_r10');ap.add_argument('--landing-time',type=float,default=11.7);args=ap.parse_args();main(args.folder,args.output_dir,args.name,args.landing_time)
