...
def main(mode):
    assert mode.startswith('r30-')
    host=ROOT/'.Codex/r30-native-server';host.mkdir(parents=True,exist_ok=True)
    ...
    cmd=[str(ROOT/'gradlew.bat'),'--no-daemon','writeServerLaunchR17','-PstrictHighDetail=true','-PoptimizedServer','-PregionalBuild='+mode,'-PreviewServerWorld=SEELE_FIELD_R30_REVIEW','-PreviewServerDirectory=.Codex/r30-native-server']
    with (ART/(mode+'.log')).open('w',encoding='utf8') as log:
        ...
        command=[('-Xmx8G' if s.startswith('-Xmx') else s) for s in spec['command']];command.insert(1,'-XX:ActiveProcessorCount=4');args=ROOT/'.Codex/r30-native-server.args'
        def quote(s):
    ...
    report={'r30-shutdown':'r30_shutdown_review.json','r30-airlift':'r30_airlift_review.json'}.get(mode)
    if report:
        outcome=json.loads((ROOT/'run/saves/SEELE_FIELD_R30_REVIEW'/report).read_text())
        assert outcome.get('passed'),outcome.get('error',outcome)
    print('Native R30 server finished',mode,flush=True)

if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('mode');main(ap.parse_args().mode)
