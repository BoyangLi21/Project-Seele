...
public final class RegionalTransitAuthor
{
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        if(!simulator.rails.isEmpty()||!simulator.stations.isEmpty())throw new IllegalStateException("Use a new staging directory; never replace an existing railway");
        ...
        {
            JsonObject j=entry.getAsJsonObject();Station station=new Station(simulator);
            station.setName(j.get("name").getAsString());station.setColor(integer(j,"color",0x52676A));
            station.setCorners(pos(j.getAsJsonArray("min")),pos(j.getAsJsonArray("max")));metadata.addStation(station);
        ...
        ObjectArrayList<Depot> depots=new ObjectArrayList<>();Map<String,Siding> lineSidings=new LinkedHashMap<>();JsonArray lineIds=new JsonArray();
        for(JsonElement entry:plan.getAsJsonArray("lines"))
        {
            ...
            metadata.addRoute(route).addDepot(depot);depots.add(depot);lineSidings.put(key,siding);
        ...
        metadata.update();simulator.setGameTime(0,1200000,true);Depot.generateDepots(simulator,depots);
        ...
        for(int tick=0;tick<5000;tick++)
        {
            ...
            if(!deployed && ready && tick>50){simulator.instantDeployDepots(depots);deployed=true;}
            if(tick%100==0)
            {
        ...
        {
            Depot depot=depots.get(i);String key=plan.getAsJsonArray("lines").get(i).getAsJsonObject().get("id").getAsString();
