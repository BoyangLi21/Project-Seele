"""Install only verified R25 cells, new lift groups and the one retired unused rail."""
...
ROOT=v.ROOT;ART=ROOT/'artifacts/facility_r25';OUT=ART/'promotion'
MAIN=ROOT/'run/saves/SEELE_TV_WORLD_PREVIEW_20260906';REVIEW=ROOT/'run/saves/SEELE_R25_REVIEW'
EDIT_ROOTS=('command_links','lifts','route_repairs','walkways','junction_finish','lift_port_finish','station_lift_port','station_platform_port','station_maps','map_finish','map_visibility','signs','sign_clearance','transfer_signs','port_finish','power_ports','edge_finish','crossing_finish','guard_finish','skin_finish')
NEW_GROUPS={'70;253','-26;-278'}
CAP=Path('dimensions/projectseele/geofront/data/capabilities.dat')
NATIVE_BOXES=[(69,-450,249,77,-386,260),(-33,-396,-285,-25,-361,-274)]
...
def protected():
 ...
 rail=json.loads((ART/'transit/installed_review.json').read_text())['retired_file']
 excluded={str(CAP).replace('\\','/'),'mtr/'+rail}
...
def main(apply=False):
 ...
 if apply:
  assert not (OUT/'installed.json').exists(),'R25 already installed'
 ...
 try:
  ...
  for (cx,cz),ids in mask.items():
   for sy in selected[cx,cz]:
    ...
    for raw in ids[ids//4096==sy]%4096:
     ...
     if old!=new:
      p.match((*q,*q),old,new,'r25/verified_map_and_native_mechanisms');cells+=1
      ...
      p.update_block_entity(q,old,mt.get(q),nt[q],'r25/verified_sign_or_control');nbt_only+=1
  ...
  rail=json.loads((ART/'transit/installed_review.json').read_text());rail_path=(MAIN/'mtr'/rail['retired_file']).resolve()
  assert rail_path.is_relative_to((MAIN/'mtr').resolve()),'Rail retirement left the selected world'
  assert rail_path.is_file() and digest(rail_path)==rail['old_sha'],'Unused-rail precondition changed'
  assert not (REVIEW/'mtr'/rail['retired_file']).exists(),'Review reintroduced the retired spur'
  p.meta.update(cells=cells,nbt_only=nbt_only,receipts=receipts,new_lift_groups=sorted(NEW_GROUPS),retired_rail=rail['retired_file'])
  p.save_plan('verified_r25_delta');print('R25 preflight',cells,'cells',nbt_only,'NBT-only',len(mask),'chunks',flush=True)
  if not apply:return
  payloads={name:(REVIEW/name).read_bytes() for name in ('nerv_routes_r24.json.gz','regional_states.json','facility_lifts_r25.json','native_transit_r25.json')}
  ...
  targets.update(MAIN/name for name in payloads);targets.update({MAIN/CAP,rail_path,MAIN/'r25_ready.json'});saved={};absent=[]
  ...
  try:
   result=p.apply('verified_r25_delta',session_lock=locks[0]);assert result['counts']['cells']==cells
   for name,data in payloads.items():atomic_replace(MAIN/name,data)
   capfile=OUT/'new_capabilities.dat';maincap.save(capfile);atomic_replace(MAIN/CAP,capfile.read_bytes());rail_path.unlink()
   check=nbtlib.load(MAIN/CAP)['data'][capkey]
   assert set(check)==set(original_groups)|NEW_GROUPS
   assert all(check[k]==value for k,value in original_groups.items()),'Existing native lift state changed'
   assert protected()==keep,'Player, actor, campaign or unrelated transport data changed'
   receipt=dict(installed=True,world=str(MAIN),cells=cells,nbt_only=nbt_only,chunks=len(mask),new_groups=sorted(NEW_GROUPS),retired_rail=rail['retired_file'],backup=str(backup),protected_data_unchanged=True)
   atomic_replace(MAIN/'r25_ready.json',json.dumps(dict(ready=True,**receipt),indent=2).encode())
   (OUT/'installed.json').write_text(json.dumps(receipt,indent=2),encoding='utf8');print('R25 installed',receipt,flush=True)
