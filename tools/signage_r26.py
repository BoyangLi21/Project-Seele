...
ROOT=v.ROOT;WORLD=ROOT/'run/saves/SEELE_R26_REVIEW';ART=ROOT/'artifacts/facility_r26';OUT=ART/'signage'
...
def main(refresh=False):
 ...
 def guidance(point,face):
  ...
  for label,key in [('指挥室','command'),('机库','hangars'),('总部车站','station')]:
   ...
   out.append(arrow+' '+('直梯 '+str(lift)+' → ' if lift is not None else '')+label)
  return out
 ...
 for q,old in iter_block_entities(WORLD,v.DIM,(-80,-474,230),(158,-355,475)):
  ...
  tag=copy.deepcopy(old);rows=guidance(nodes[node,:3],face)
 ...
 for point,face,label in ([] if refresh else sites):
  ...
  place(nodes[node,:3].tolist(),'金字塔 · 楼层导引',str(int(nodes[node,1]))+' 层',guidance,face,compact=True)
