...
ROOT=base.ROOT;OUT=ROOT/'artifacts/server-ready-r27';STAGE=OUT/'stage';TEST=OUT/'production-run'

def main(resume=False):
    if resume:
        assert base.sha256(next((TEST/'mods').glob('projectseele-*.jar')))==base.sha256(next((STAGE/'server/mods').glob('projectseele-*.jar')))
        assert (TEST/base.WORLD_NAME/'R27_BATCH.json').read_bytes()==(STAGE/'world/R27_BATCH.json').read_bytes()
    else:
        ...
        while True:
            ...
            if line:
                ...
                if 'Done (' in line and ready_at is None:
                    ...
                    proc.stdin.write('execute in projectseele:geofront run forceload add -12 -75\nexecute in projectseele:geofront run forceload add 27 277\n');proc.stdin.flush()
            if ready_at and not witness and time.monotonic()-ready_at>12:
                proc.stdin.write('execute in projectseele:geofront run data get block -12 81 -75\nexecute in projectseele:geofront as @e[type=projectseele:nerv_staff,nbt={StaffId:"fuyutsuki"},x=27,y=-406,z=277,distance=..2] run say R27_FUYUTSUKI_POST_LOADED\nexecute in projectseele:geofront run time query daytime\n');proc.stdin.flush();witness=True
