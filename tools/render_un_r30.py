...
ap=argparse.ArgumentParser();ap.add_argument('--unit',required=True);ap.add_argument('--views',default='front,threequarter,rear');ap.add_argument('--pose',choices=('rest','fist'),default='rest');ap.add_argument('--open',type=float,default=0);ap.add_argument('--asset-root',type=Path,default=ROOT/'artifacts/facility_r30/models');a=ap.parse_args(sys.argv[sys.argv.index('--')+1:])
...
def transform(name):
 if name not in matrices:
  b=bones[name];p=Vector(np.asarray(b['pivot'])*[-1,1,1]);angles=np.asarray(b.get('rotation',[0,0,0]),float)*[-1,-1,1]
  if a.pose=='fist' and name.startswith('finger_thumb_axis_'):angles[2]+=100 if name.endswith('_r') else -100
  if a.pose=='fist' and name.startswith('finger_') and '_axis_' not in name:
   joint=2 if '_distal_' in name else 1 if '_tip_' in name else 0;angles[2]+=(45,60,0)[joint] if 'thumb' in name else (70,105,70)[joint]
  q=Euler(tuple(np.deg2rad(angles)),'XYZ').to_matrix().to_4x4()
...
mr=material.node_tree.nodes.new('ShaderNodeTexImage');mr.image=bpy.data.images.load(str(ASSET/'textures/entity/eva_prototype_mr.png'));mr.image.colorspace_settings.name='Non-Color';mr.image.pack();split=material.node_tree.nodes.new('ShaderNodeSeparateColor');split.mode='RGB';material.node_tree.links.new(mr.outputs['Color'],split.inputs[0]);material.node_tree.links.new(split.outputs['Green'],shader.inputs['Roughness']);material.node_tree.links.new(split.outputs['Blue'],shader.inputs['Metallic'])
eyes=material.node_tree.nodes.new('ShaderNodeTexImage');eyes.image=bpy.data.images.load(str(ASSET/'textures/entity/eva_prototype_eyes.png'));eyes.image.pack();material.node_tree.links.new(eyes.outputs['Color'],shader.inputs['Emission Color']);shader.inputs['Emission Strength'].default_value=1.2
...
views.update(head=(0,48,55),hand_r=(20,35,29),hand_l=(-20,35,29))
for index,name in enumerate(filter(None,a.views.split(','))):
 target=Vector((0,0,49 if name=='dorsal' else 30));camera.location=views[name];camera.data.ortho_scale=21 if name=='dorsal' else 67
 if name=='head':target=Vector((0,0,55.2));camera.data.ortho_scale=14
 if name.startswith('hand_'):
  p=np.asarray(bones[name]['pivot'])*[-1,1,1]*5/16;target=Vector((p[0],-p[2],p[1]-2.5));camera.location=target+Vector((5 if name.endswith('r') else -5,24,4));camera.data.ortho_scale=9
 camera.rotation_euler=(target-camera.location).to_track_quat('-Z','Y').to_euler()
 if index==0:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'runtime_inspection.blend'))
 scene.render.filepath=str(OUT/('runtime_'+name+('_'+a.pose if a.pose!='rest' else '')+('_open' if a.open else '')+'.png'));bpy.ops.render.render(write_still=True)
