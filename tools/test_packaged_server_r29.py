...
ROOT=base.ROOT;OUT=ROOT/'artifacts/server-ready-r29';STAGE=OUT/'stage';TEST=OUT/'production-run'

def main(resume=False):
    if resume:
        assert base.sha256(next((TEST/'mods').glob('projectseele-*.jar')))==base.sha256(next((STAGE/'server/mods').glob('projectseele-*.jar')))
        assert (TEST/base.WORLD_NAME/'R29_BATCH.json').read_bytes()==(STAGE/'world/R29_BATCH.json').read_bytes()
    else:
    ...
    with (OUT/'production_server.log').open('w',encoding='utf8') as log:
        while True:
            ...
            if line:
                ...
                if 'R29_FUYUTSUKI_POST_LOADED' in line:commander=True
