...
ROOT=Path(__file__).resolve().parents[1];ART=ROOT/'artifacts/facility_r26';WORLD=ROOT/'run/saves/SEELE_R26_REVIEW';OUT=ART/'navigation'
...
def main():
    ...
    nav.PUBLIC_DOMAINS=[((-76,-474,235),(157,-356,474)),((-34,-369,-288),(115,-360,-215))];nav.PUBLIC_PATHS=paths
    ...
    for y in (-461,-448,-434,-420,-406,-392,-378,-364):
        ...
        for room in [q for q in rooms if q['floor']+1==y]:
            ...
            if connected:
                ...
                compact.append(points[-1]);routes.append(dict(id='r26/room_to_floor_lift/'+room['id'],path=compact))
