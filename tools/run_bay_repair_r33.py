...
def main(serial=-1,visual=False,wet=False,shader=None):
    ...
    command.insert(1,'-Dprojectseele.facilityWetReview='+str(wet).lower())
    ...
    with temporary_options(ROOT/'run/options.txt',':',{'renderDistance':'6','fov':'0.625'}):
        with temporary_options(ROOT/'run/config/oculus.properties','=',{'enableShaders':'true' if shader else 'false',**({'shaderPack':shader} if shader else {})}):
...
if __name__=='__main__':
    import argparse
    p=argparse.ArgumentParser();p.add_argument('--serial',type=int,default=-1,choices=[-1,0,1]);p.add_argument('--visual-only',action='store_true');p.add_argument('--wet-view',action='store_true');p.add_argument('--shader');a=p.parse_args();main(a.serial,a.visual_only,a.wet_view,a.shader)
