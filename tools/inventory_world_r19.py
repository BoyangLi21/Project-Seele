...
def main():
    ...
    results={k:[] for k in ('signs','buttons','ticket_machines','wooden_seats','escalators')}
    sections=0;chunks=set();start=time.monotonic();census=Counter()
    ...
    report={'world':str(WORLD),'regions':len(regions),'full_chunks':len(chunks),'sections':sections,'census':dict(census),'seconds':round(time.monotonic()-start,1),'objects':results}
