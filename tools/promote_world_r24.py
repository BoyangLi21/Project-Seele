"""Install verified R24 voxel/NBT deltas without transplanting the test world.

Only named R24 edit receipts contribute coordinates. Player, entity, transport,
...
ROOT=v.ROOT;ART=ROOT/'artifacts/facility_r24';OUT=ART/'promotion'
MAIN=ROOT/'run/saves/SEELE_TV_WORLD_PREVIEW_20260906';REVIEW=ROOT/'run/saves/SEELE_R24_TV_REVIEW'
EDIT_ROOTS=('stations','station_decks','shops','shop_signs','fixture_parts','wayfinding_labels','station_civil_details','materials','pyramid_life','kirisato_life','city_frontages','residue')
...
def protected_hashes():
    ...
    return {str(p.relative_to(MAIN)):digest(p) for p in paths if p.exists()}

def main(apply=False):
    OUT.mkdir(parents=True,exist_ok=True)
    if apply:
        assert not (OUT/'installed.json').exists(),'R24 already installed; write a follow-up patch'
    ...
    try:
        ...
        for (cx,cz),ids in sorted(mask.items()):
            for sy in sorted(selected[cx,cz]):
                ...
                for n in ids[ids//4096==sy]%4096:
                    ...
                    if old!=new:
                        p.match((*q,*q),old,new,'r24/verified_geometry');cells+=1
                        ...
                        p.update_block_entity(q,old,mt,nt,'r24/verified_wayfinding_text');nbt_only+=1
        ...
        p.meta.update(expected_cells=cells,expected_nbt_only=nbt_only,source_receipts=receipts,main_player_entity_transport_state_preserved=True)
        p.save_plan('verified_r24_cells_and_signs')
        print('R24 preflight',cells,'voxel cells',nbt_only,'NBT-only cells',len(mask),'chunks',flush=True)
        if not apply:return
        protected=protected_hashes();stamp=datetime.datetime.now().strftime('%Y%m%d_%H%M%S');backup=OUT/('main_before_'+stamp);saved={};absent=[]
        metadata={'nerv_routes_r24.json.gz':REVIEW/'nerv_routes_r24.json.gz'}
        # The main route catalogue keeps all existing routes and adds the
        # actually verified R24 approach paths, deduplicated by stable ID.
        routes=json.loads((MAIN/'quality_walk_cases.json').read_text(encoding='utf8'))
        union={r['id']:r for r in routes}
        union.update({r['id']:r for r in json.loads((REVIEW/'r24_walk_cases.json').read_text(encoding='utf8'))})
        ...
        targets.update(MAIN/name for name in payloads);targets.add(MAIN/'r24_ready.json')
        ...
        try:
            result=p.apply('verified_r24_cells_and_signs',session_lock=locks[0]);assert result['counts']['cells']==cells
            ...
            receipt=dict(installed=True,world=str(MAIN),source=str(REVIEW),changed_cells=cells,nbt_only_cells=nbt_only,chunks=len(mask),route_catalog=len(union),backup=str(backup),protected_hashes_unchanged=True,private_pack_unchanged=True)
            atomic_replace(MAIN/'r24_ready.json',json.dumps(dict(ready=True,**receipt),indent=2).encode('utf8'))
            (OUT/'installed.json').write_text(json.dumps(receipt,indent=2),encoding='utf8');print('R24 installed',receipt,flush=True)
