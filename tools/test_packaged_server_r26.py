...
ROOT=base.ROOT;OUT=ROOT/'artifacts/server-ready-r26';STAGE=OUT/'stage';TEST=OUT/'production-run'

def main(resume=False):
    if resume:
        assert base.sha256(next((TEST/'mods').glob('projectseele-*.jar')))==base.sha256(next((STAGE/'server/mods').glob('projectseele-*.jar')))
        assert (TEST/base.WORLD_NAME/'R26_BATCH.json').read_bytes()==(STAGE/'world/R26_BATCH.json').read_bytes()
    else:
    ...
    proof={'passed':all(checks.values()),'checks':checks,'mod_sha256':base.sha256(next((TEST/'mods').glob('projectseele-*.jar'))),'scope':'Reobfuscated private R26 JAR, production Forge 47.4.10, imported complete world, loaded GeoFront pylon, clean shutdown; loopback-only, 5 GiB test heap. No remote client login asserted.'}
