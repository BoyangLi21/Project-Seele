"""Stage, verify and seal the three matching private R25 deployment archives."""
from pathlib import Path
import argparse,datetime,hashlib,json,msvcrt,shutil,subprocess,zipfile
import build_server_ready_pack as base
ROOT=base.ROOT;OUT=ROOT/'artifacts/server-ready-r25';STAGE=OUT/'stage'
...
def stage():
    ...
    assert json.loads((ROOT/'artifacts/facility_r25/promotion/installed.json').read_text())['installed']
    ...
    scripts=ROOT/'tools/deployment_r25'
    ...
    manifest={'revision':'R25','created':datetime.datetime.now().astimezone().isoformat(),'source_head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'world':base.WORLD_NAME,'mod_sha256':base.sha256(next((server/'mods').glob('projectseele-*.jar'))),'world_marker_sha256':base.sha256(world/'r25_ready.json')}
    assert manifest['mod_sha256']==base.sha256(next((client/'mods').glob('projectseele-*.jar'))),'The two private JARs differ'
    for root in (server,client,world):write(root/'R25_BATCH.json',json.dumps(manifest,ensure_ascii=False,indent=2))
    ...
    print('R25 staged at',STAGE,flush=True)
...
def seal():
    ...
    for root in (server,client,world):write(root/'R25_BATCH.json',json.dumps(batch,ensure_ascii=False,indent=2))
    for root,kind in ((server,'server-files'),(client,'client-pack')):base.write_manifest(root,kind)
    write(OUT/'batch.json',json.dumps(batch,indent=2))
    paths=[OUT/'Project_SEELE_R25_Server.zip',OUT/'Project_SEELE_R25_World.zip',OUT/'Project_SEELE_R25_Client.zip']
    ...
    write(OUT/'RELEASE.json',json.dumps({'revision':'R25','files':rows,'production_test':proof},ensure_ascii=False,indent=2))
