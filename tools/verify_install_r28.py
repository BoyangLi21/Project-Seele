...
def main():
    ...
    with (p.MAIN/'session.lock').open('r+b') as lock:
        ...
        native=json.loads((p.ART/'airport/native_install.json').read_text())
        for name,sha in native['after'].items():
            path=p.MAIN/'mtr'/name;assert (p.digest(path) if path.exists() else None)==sha,name
        ...
        archive=p.ROOT/'artifacts/server-ready-r27/Project_SEELE_R27_Client.zip'
