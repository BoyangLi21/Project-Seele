#!/usr/bin/env python3
"""Restore the second original spruce shaft door at 24,-423,254."""
...
PACKET = "S45-RESTORE-SECOND-ORIGINAL-SHAFT-DOOR-R01"
...
def plan(world: Path) -> tuple[list[Change], dict]:
    ...
    door41 = next(door for door in receipt41["doors"] if door["id"] == DOOR_ID)
    door42 = next(door for door in receipt42["doors"] if door["id"] == DOOR_ID)
    ...
    changes = [Change(
        ...
        "restore_second_original_single_spruce_shaft_door")
    ...
    marker["doors"] = [door for door in marker["doors"]
                       if int(door["id"]) != DOOR_ID]
    excluded = marker.setdefault("excludedOriginalDoors", [])
    entry = {
        "lower": [24, -423, 254],
...
def apply(world: Path, changes: list[Change], marker: dict) -> Path:
    ...
    artifact = ROOT / "artifacts" / f"s45_restore_second_shaft_door_{stamp}"
    ...
    (world / MARKER).write_text(json.dumps(marker, indent=2) + "\n",
                               encoding="utf-8")
...
def main() -> None:
    ...
    if args.apply:
        result.update(applied=True, artifact=str(apply(world, changes, marker)))
