"""Stage, verify and seal the three matching private R29 deployment archives."""
from pathlib import Path
import argparse,datetime,hashlib,json,msvcrt,shutil,subprocess,zipfile
import build_server_ready_pack as base
ROOT=base.ROOT;OUT=ROOT/'artifacts/server-ready-r29';STAGE=OUT/'stage'
...
def stage():
    ...
    assert json.loads((ROOT/'artifacts/facility_r29/promotion/installed.json').read_text())['installed']
    ...
    base.build_server(server,guide,revision='R29');base.build_client(client,guide,revision='R29',city_shaders=True)
    ...
    for folder in (server,client):
        for name in ('MANUAL_ACCEPTANCE_R23.md','MANUAL_ACCEPTANCE_R24.md','MANUAL_ACCEPTANCE_R26.md','FIRST_ACT_R29_CN.md','FIELD_REPAIR_R29.md','PRIVATE_SERVER_DEPLOYMENT_CN.md'):
    ...
    write(client/'Install-Visuals.bat','@echo off\r\ncd /d "%~dp0"\r\npowershell -NoProfile -ExecutionPolicy Bypass -File Install-Visuals.ps1\r\npause\r\n')
    ...
    manifest={'revision':'R29','created':datetime.datetime.now().astimezone().isoformat(),'source_head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'world':base.WORLD_NAME,'mod_sha256':base.sha256(next((server/'mods').glob('projectseele-*.jar'))),'world_marker_sha256':base.sha256(world/'r29_ready.json')}
    assert manifest['mod_sha256']==base.sha256(next((client/'mods').glob('projectseele-*.jar'))),'The two private JARs differ'
    for root in (server,client,world):write(root/'R29_BATCH.json',json.dumps(manifest,ensure_ascii=False,indent=2))
    ...
    print('R29 staged at',STAGE,flush=True)
...
def seal():
    ...
    assert not list(client.glob('shaderpacks/*.zip')),'Shader archive must be downloaded from its author'
    ...
    for root in (server,client,world):write(root/'R29_BATCH.json',json.dumps(batch,ensure_ascii=False,indent=2))
    for root,kind in ((server,'server-files'),(client,'client-pack')):base.write_manifest(root,kind)
    write(OUT/'batch.json',json.dumps(batch,indent=2))
    paths=[OUT/'Project_SEELE_R29_Server.zip',OUT/'Project_SEELE_R29_World.zip',OUT/'Project_SEELE_R29_Client.zip']
    for root,path in zip((server,world,client),paths):base.zip_tree(root,path)
    base.validate_outputs(*paths,city_shaders=True)
    ...
    write(OUT/'RELEASE.json',json.dumps({'revision':'R29','files':rows,'production_test':proof},ensure_ascii=False,indent=2))
