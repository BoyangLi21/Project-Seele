"""Cold full-height audit of the current R24 save, using canonical readers."""
from pathlib import Path
import json,msvcrt,os,subprocess,sys
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'artifacts/facility_r24/global';WORLD=ROOT/'run/saves/SEELE_R24_TV_REVIEW'
...
def main():
    ...
    with (WORLD/'session.lock').open('r+b') as lock:
        ...
        snapshot=dict(scan=dict(full_chunks=inventory['full_chunks'],sections=inventory['sections'],complete=True,source='Fresh R24 full-height save scan; includes surface and underground'),
    ...
    print('R24 current whole-height scan complete; candidates still require semantic classification',flush=True)
