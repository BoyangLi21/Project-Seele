...
ROOT=m.ROOT;ROOT_OUT=ROOT/'artifacts/access_r22/models';INSTALLED=Path(json.loads((ROOT/'artifacts/access_r22/baseline.json').read_text())['backup'])/'eva_real_model/assets/projectseele'
...
def main(unit):
 ...
 geo=json.loads((INSTALLED/'geo/eva_prototype.geo.json').read_text());bones=geo['minecraft:geometry'][0]['bones'];bones=[copy.deepcopy(b) for b in bones if not b['name'].startswith(('dorsal_','r21_join_'))]
 ...
 labels=np.array([classify(q) for q in raw]);old_labels=labels.copy()
 # Recover anatomical branches along the actual mesh surface. Low outer
 # knee armour is closer to the leg along the mesh, even when its X/Y box
 # overlaps the old forearm classifier. Hands remain attached to arm seeds.
 from scipy.sparse.csgraph import dijkstra
 edges=np.vstack([faces[:,[0,1]],faces[:,[1,2]],faces[:,[2,0]]]);edges=np.vstack([edges,edges[:,::-1]])
 lengths=np.maximum(1e-5,np.linalg.norm(raw[edges[:,0]]-raw[edges[:,1]],axis=1))
 graph=sparse.coo_matrix((lengths,(edges[:,0],edges[:,1])),shape=(len(raw),len(raw))).tocsr()
 ax=np.abs(raw[:,0]);yy=raw[:,1];edge=23 if unit=='00' else 30
 arm_seeds=np.flatnonzero((ax>edge+2)&(yy>114)&(yy<135))
 leg_seeds=np.flatnonzero(((yy>24)&(yy<64)&(ax>7))|((yy>=64)&(yy<111)&(ax>7)&(ax<edge-3)))
 assert len(arm_seeds)>10 and len(leg_seeds)>10
 da=dijkstra(graph,indices=arm_seeds,min_only=True);dl=dijkstra(graph,indices=leg_seeds,min_only=True)
 ambiguous=(yy>=64)&(yy<114)&(ax>7)
 leg=ambiguous&(dl<da)
 arm=ambiguous&(da<dl)&(ax>edge-2)
 for side,sign in [('l',-1),('r',1)]:
  mask=raw[:,0]*sign>0
  labels[leg&mask]='leg_'+side
  labels[arm&mask]='forearm_'+side
 arm_family=np.isin(labels,['arm_l','arm_r','forearm_l','forearm_r'])
 leg_family=np.isin(labels,['leg_l','leg_r','shin_l','shin_r','foot_l','foot_r'])
 webs=arm_family[faces].any(1)&leg_family[faces].any(1)
 anatomy=dict(reclassified_vertices=int(np.count_nonzero(labels!=old_labels)),old_forearm_knee_vertices=int(np.count_nonzero(np.char.startswith(old_labels,'forearm')&(yy<85))),new_forearm_knee_vertices=int(np.count_nonzero(np.char.startswith(labels,'forearm')&(yy<85)&(dl<da))),removed_arm_leg_web_triangles=int(webs.sum()))
 faces=faces[~webs];colours=colours[~webs]
 (out/'anatomy.json').write_text(json.dumps(anatomy,indent=2));assert anatomy['new_forearm_knee_vertices']==0
 names=sorted(set(labels));indices={n:i for i,n in enumerate(names)}
 ...
 centre=raw[faces].mean(1);hand_faces=np.char.startswith(labels[faces],'forearm').all(1)&(centre[:,1]<wrist);kept=~hand_faces
 ...
 mesh=dict(format_version=1,model_height=193.1,stride=8,atlas_rows=3,triangleCount=sum(len(x['vertices'])//24 for x in parts.values()),parts=parts,jointSkins=seam_specs,eye_socket_model=lens.tolist(),source='Lux3D curved armour, R22 geodesic limb separation, original hands and surface-projected marks',lux3d_task='3569385' if unit=='00' else '3569389')
