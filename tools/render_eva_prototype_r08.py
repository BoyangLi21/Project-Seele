...
ROOT=Path('D:/eva');OUT=ROOT/'artifacts/world_refinement_r08/prototype'
d=np.load(OUT/'model.npz');v=d['vertices'][:,[0,2,1]]*[1,-1,1]/16*5
bpy.ops.object.select_all(action='SELECT');bpy.ops.object.delete(use_global=False)
me=bpy.data.meshes.new('R08 concept-matched prototype');me.from_pydata(v,[],np.arange(len(v)).reshape(-1,3));me.update()
...
for i,colour in enumerate(d['colours']):
    rgb=[int(colour[j:j+2],16)/255 for j in (1,3,5)];rgb=[x/12.92 if x<=.04045 else ((x+.055)/1.055)**2.4 for x in rgb];mat=bpy.data.materials.new('Finish '+str(i));mat.diffuse_color=(*rgb,1);mat.use_nodes=True
    ...
    bsdf.inputs['Base Color'].default_value=(*rgb,1);bsdf.inputs['Roughness'].default_value=.42 if i==0 else .65;bsdf.inputs['Metallic'].default_value=.82 if i in (4,6) else .6 if i==2 else .15
...
for name,loc in [('front',(0,110,35)),('threequarter',(77,115,49)),('profile',(110,0,35)),('rear',(0,-110,35))]:
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'eva_prototype_r08.blend'))
