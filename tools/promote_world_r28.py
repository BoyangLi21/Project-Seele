"""Install verified R28 voxel/NBT deltas without transplanting the test world.

Only named R28 edit receipts contribute coordinates. Player, entity, transport,
...
ROOT=v.ROOT;ART=ROOT/'artifacts/facility_r28';OUT=ART/'promotion'
MAIN=ROOT/'run/saves/SEELE_TV_WORLD_PREVIEW_20260906';REVIEW=ROOT/'run/saves/SEELE_FIELD_R28_REVIEW'
EDIT_ROOTS=('reported','airport/civil','power','retired_transit','enclosures','science_underpass','science_roof_restore','junction_fixes','train_repairs','finish','signage','rack_port','rail_corner','retained_junctions','stair_heads','map_access','hakone_map','airport_piers')
...
def protected_hashes():
    ...
    allowed={'mtr/'+k for k in json.loads((ART/'airport/native_install.json').read_text())['delta']}
    return {p.relative_to(MAIN).as_posix():digest(p) for p in paths if p.exists() and p.relative_to(MAIN).as_posix() not in allowed}

def main(apply=False):
    OUT.mkdir(parents=True,exist_ok=True)
    if apply:
        assert not (OUT/'installed.json').exists(),'R28 already installed; write a follow-up patch'
    ...
    try:
        ...
        for (cx,cz),ids in sorted(mask.items()):
            for sy in sorted(selected[cx,cz]):
                ...
                for n in ids[ids//4096==sy]%4096:
                    ...
                    if old!=new:
                        p.match((*q,*q),old,new,'r28/verified_geometry');cells+=1
                        ...
                        p.update_block_entity(q,old,mt,nt,'r28/verified_wayfinding_text');nbt_only+=1
        ...
        p.save_plan('verified_r28_cells_and_signs')
        print('R28 preflight',cells,'voxel cells',nbt_only,'NBT-only cells',len(mask),'chunks',flush=True)
        if not apply:return
        protected=protected_hashes();stamp=datetime.datetime.now().strftime('%Y%m%d_%H%M%S');backup=OUT/('main_before_'+stamp);saved={};absent=[]
        metadata={name:REVIEW/name for name in ('nerv_routes_r24.json.gz','r28_surface_power.json','native_transit_r28.json','native_transit_r26.json')}
        # The main route catalogue keeps all existing routes and adds the
        # actually verified R28 approach paths, deduplicated by stable ID.
        routes=json.loads((MAIN/'quality_walk_cases.json').read_text(encoding='utf8'))
        union={r['id']:r for r in routes}
        union.update({r['id']:r for r in json.loads((REVIEW/'r28_walk_cases.json').read_text(encoding='utf8'))})
        ...
        native=json.loads((ART/'airport/native_install.json').read_text());native_stage=ROOT/'.Codex/r28-native-airport'
        for name in native['delta']:
            target=MAIN/'mtr'/name;expected=native['before'][name]
            assert (digest(target) if target.exists() else None)==expected,('Authoritative railway changed',name)
            targets.add(target)
        targets.update(MAIN/name for name in payloads);targets.add(MAIN/'r28_ready.json')
        ...
        try:
            result=p.apply('verified_r28_cells_and_signs',session_lock=locks[0]);assert result['counts']['cells']==cells
            for name,data in payloads.items():atomic_replace(MAIN/name,data)
            for name in native['delta']:
                target=(MAIN/'mtr'/name).resolve();assert target.is_relative_to((MAIN/'mtr').resolve())
                if native['after'][name] is None:
                    if target.is_file():target.unlink()
                else:
                    source=native_stage/name;assert digest(source)==native['after'][name]
                    target.parent.mkdir(parents=True,exist_ok=True);atomic_replace(target,source.read_bytes())
            assert protected_hashes()==protected,'Player, entity, transport or mechanical state changed'
            receipt=dict(installed=True,world=str(MAIN),source=str(REVIEW),changed_cells=cells,nbt_only_cells=nbt_only,chunks=len(mask),route_catalog=len(union),backup=str(backup),protected_hashes_unchanged=True,private_pack_unchanged=True,native_s1_files=len(native['delta']))
            atomic_replace(MAIN/'r28_ready.json',json.dumps(dict(ready=True,**receipt),indent=2).encode('utf8'))
            (OUT/'installed.json').write_text(json.dumps(receipt,indent=2),encoding='utf8');print('R28 installed',receipt,flush=True)
