@echo off
setlocal
title Project SEELE - TV world acceptance R28
pushd "%~dp0"
...
if not exist "run\saves\SEELE_TV_WORLD_PREVIEW_20260906\r28_ready.json" (
    echo ERROR: R28 manual handoff is not ready yet.
    goto failed
)
...
"%SEELE_PYTHON%" "tools\fetch_realistic_pack_r25.py" --enable
if errorlevel 1 goto failed

echo Project SEELE - R28 installed TV world, local manual acceptance
echo World: SEELE_TV_WORLD_PREVIEW_20260906
echo Client heap: 6 GB. Exact terrain: 24 chunks. LOD: disabled.
echo Guide: %CD%\docs\MANUAL_ACCEPTANCE_R28.md
echo First act: %CD%\docs\FIRST_ACT_R28_CN.md
...
:usage
echo Usage: start_eva_test_r28.bat [--check]
