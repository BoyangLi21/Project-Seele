@echo off
setlocal
title Project SEELE - TV world acceptance R24
pushd "%~dp0"
...
if not exist "run\saves\SEELE_TV_WORLD_PREVIEW_20260906\r24_ready.json" (
    echo ERROR: R24 manual handoff is not ready yet.
    goto failed
)
...
echo Project SEELE - R24 installed TV world, local manual acceptance
echo World: SEELE_TV_WORLD_PREVIEW_20260906
echo Client heap: 6 GB. Exact terrain: 24 chunks. LOD: disabled.
echo Guide: %CD%\docs\MANUAL_ACCEPTANCE_R24.md
...
:usage
echo Usage: start_eva_test_r24.bat [--check]
