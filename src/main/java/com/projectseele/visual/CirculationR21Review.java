...
public final class CirculationR21Review
{
 ...
 private static final boolean R23="r23-edges".equals(System.getProperty("projectseele.regionalBuild",""));
 ...
 @SubscribeEvent public static void tick(TickEvent.ServerTickEvent e)
 {
  ...
  var world=e.getServer().getWorldPath(LevelResource.ROOT).normalize();if(!world.getFileName().toString().equals(R23?"SEELE_R22_REVIEW":"SEELE_R21_REVIEW"))throw new IllegalStateException("Edge audit boundary");
  ...
  try
  {
   ...
   if(R23)
   {
    var data=JsonParser.parseString(Files.readString(world.resolve("r23_guard_cases.json"))).getAsJsonArray();edges=new double[data.size()][5];int row=0;
    for(var value:data){var test=value.getAsJsonObject();var p=test.getAsJsonArray("start");var d=test.getAsJsonArray("direction");edges[row++]=new double[]{p.get(0).getAsDouble(),p.get(1).getAsDouble(),p.get(2).getAsDouble(),d.get(0).getAsDouble(),d.get(2).getAsDouble()};}
   }
   for(double[] q:edges)
   {
   ...
   var report=new JsonObject();report.addProperty("passed",passed);report.add("checks",results);Files.writeString(world.resolve(R23?"r23_edge_physics.json":UN_BAYS?"r21_un_edge_physics.json":"r21_edge_physics.json"),report.toString());ProjectSeele.LOGGER.info("R21 EDGE PHYSICS {}",report);
  }
