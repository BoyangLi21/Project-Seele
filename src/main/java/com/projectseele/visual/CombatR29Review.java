...
{
    public static final boolean ENABLED="r29-combat".equals(System.getProperty("projectseele.regionalBuild",""));
    ...
    {
        if(!ENABLED||!ready||done||!ShamshelContactR24Review.completed||e.phase!=TickEvent.Phase.END)return;
        var server=e.getServer();var world=server.getWorldPath(LevelResource.ROOT).normalize();
        if(!world.getFileName().toString().equals("SEELE_FIELD_R29_REVIEW"))throw new IllegalStateException("Combat review world boundary");
        ...
        try
        {
            if(eva==null)
            {
                ...
                eva=ModEntities.EVA_PROTOTYPE.get().create(level);eva.prepareForMotionLab();eva.setNoAi(true);eva.moveTo(32.5,81,217.5,180,0);eva.yBodyRot=180;level.addFreshEntity(eva);evaId=eva.getId();initialHealth=eva.getHealth();
            ...
            age++;
            if(age==100||age==180)angel.beginStrike(eva,age==100?1:2);
            if(age>100&&age<230&&EvaImpactResponse.sample(eva,1).energy()>.01)recoilSamples++;
            if(age==113)photo="sachiel_windup";if(age==120)photo="sachiel_contact";if(age==132)photo="eva_recovery";
            if(age==250)
            {
            ...
            if(age==290||age==350)
            {var t=new net.minecraft.nbt.CompoundTag();whip.saveWithoutId(t);t.putInt("SweepAge",0);t.putInt("SweepSide",age==290?-1:1);t.putFloat("SweepYaw",0);whip.load(t);}
            if(age==306)photo="shamshel_sweep";
            if(age==400)
            {
            ...
            if(age==445)photo="un_transport_lit_model";
            if(age==480)
            {
            ...
            if(age==505)photo="nuclear_cross_finale";
            if(age>=740)
            {
                ...
                r.addProperty("passed",handSamples>20&&maxHandError<.2&&whipSamples>20&&maxWhipError<.05&&recoilSamples>8&&eva.getHealth()<initialHealth);
                Files.writeString(world.resolve("r29_combat_visual.json"),new GsonBuilder().setPrettyPrinting().create().toJson(r));done=true;
            }
        }
