@echo off
setlocal
title Project SEELE - TV world acceptance R22
pushd "%~dp0"
...
if not exist "run\saves\SEELE_R22_REVIEW\level.dat" (
    echo ERROR: The current TV world is missing.
    goto failed
)
if not exist "run\saves\SEELE_R22_REVIEW\r22_manual_handoff.json" (
    echo ERROR: R22 manual handoff is not ready yet.
    goto failed
)
...
echo Project SEELE - R22 inspection copy, local manual acceptance
echo World: SEELE_R22_REVIEW
echo Client heap: 6 GB. Exact terrain: 24 chunks. LOD: disabled.
echo Guide: %CD%\docs\MANUAL_ACCEPTANCE_R22.md
echo Close any other Minecraft instance using this world before starting.
echo.
if "%SEELE_CHECK_ONLY%"=="1" goto check

"%SEELE_PYTHON%" "tools\launch_rendered_client_r17.py" --world SEELE_R22_REVIEW --review r22-manual --heap 6G
set "SEELE_EXIT_CODE=%ERRORLEVEL%"
if not "%SEELE_EXIT_CODE%"=="0" goto failed
goto done

:check
"%SEELE_PYTHON%" "tools\launch_rendered_client_r17.py" --world SEELE_R22_REVIEW --review r22-manual --heap 6G --prepare-only
set "SEELE_EXIT_CODE=%ERRORLEVEL%"
if not "%SEELE_EXIT_CODE%"=="0" goto failed
...
:usage
echo Usage: start_eva_test_r22.bat [--check]
