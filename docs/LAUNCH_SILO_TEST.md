...
1. **主流程：连续地图发射井。** 第三新东京市与 GeoFront 位于同一维度，三条 11×11 净空竖井真实连接地下和地表，载台行程 286 格。
...
6. 同步、舱盖闭锁后，移动载台应连续上升 286 格；中途维度和 EVA UUID 不得变化。
7. 到达地表时 EVA 基座应位于 Y=248，日志依次出现：

   ```text
   NERV launch locked
   NERV launch ascent
   NERV launch surface clear
   NERV continuous sortie complete ... rise=286
   ```
...
该批次真实执行短井 `setup` 和 `board`，按 `IDLE → LOCKED → ASCENT → SURFACE_CLEAR` 保存六张 PNG：高位背部平台、插入栓下降外景、同步驾驶舱、舱盖闭锁、井内中段上升、地表释放。它适合快速检查插入栓和局部状态机，不验收 286 格同维路线。
...
该批次必须在同一维度捕获地下待命、插入栓锁定、100～240 格中段上升和东京市地表到达。最终仍需要人类检查井壁碰撞、载台重量感、镜头舒适度、EVA 朝向和城市比例。
