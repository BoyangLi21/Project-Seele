...
连续地图会在三座地下床位旁和三座地表站旁自动部署 6 个 `projectseele:umbilical_pylon`。无人地下待命时必须是 0 电、黑眼和双臂下垂的冷机；插入栓完成 LCL/A10 准备后才接通地下脐带并从 0 充电。沿 522 格竖井进行的计划运输不会误报战损断缆，抵达同路线地表供电桩附近后可接入该站电源；真正驾驶离开默认 32 格范围或遭受重大机体损伤后，脐带掉落并切入 5 分钟内置电池。`/seele geofront link` 与实时发射准备都会补齐旧存档缺失的供电桩，`sortie_audit` 会拒绝供电点不完整的路线。
