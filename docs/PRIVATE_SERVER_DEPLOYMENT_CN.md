...
R20 还包含 `eva_facility_r20.json`、`un01_annex_r20.json`、`world_revision_r20.json` 和完整 `mtr/` 数据。请整包导入本轮正式存档，保留这些文件；三机机库已经移动到 Z=-240，发射床升至 Y=-411。包内 `R20_TEST_GUIDE_CN.md` 提供新坐标、交通乘坐和两座 UN 机库控制说明。UN-01 为已完成的空试验舱，模型仍待单独交接。
