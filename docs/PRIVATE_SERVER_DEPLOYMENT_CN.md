# Project SEELE R29 服务器部署

使用同一批 `Project_SEELE_R29_Server.zip`、`Project_SEELE_R29_World.zip` 和 `Project_SEELE_R29_Client.zip`，校验值在 `SHA256SUMS.txt`。两端 Project SEELE JAR 完全相同；本轮网络协议升为 33，请整批更新 R29 客户端和服务器以获得一致的 UN 运输、飞行、战役与地图行为；存档仍为 `SEELE_TV_WORLD_PREVIEW_20260906`。
...
3. 新建 `SEELE_TV_WORLD_PREVIEW_20260906` 子目录，将 World ZIP 内容解压进去。最终应有 `SEELE_TV_WORLD_PREVIEW_20260906/level.dat` 和 `r29_ready.json`，不要套两层同名目录。
4. 阅读 Minecraft EULA 后自行将 `eula.txt` 改为 `eula=true`。
5. 运行 `Start-Server.bat`；控制台应出现 `Project SEELE initialized` 与 `Done`。启动器在缺少 R29 存档时会停止，避免误建空世界。
...
服务器 15 个顶层 JAR；客户端再加 Embeddium、Oculus、Xaero 小地图和世界地图，共 19 个。内嵌依赖自动加载，已包含 Patchouli 载具说明书和钢琴。不要再添加另一版本 MTR、Superb Warfare、GeckoLib 或电梯模组。
...
先按 `R29_TEST_GUIDE_CN.md` 验证电梯、NPC 出动与回收、车站换乘、姿态和电源。既有 UN、载具与飞机操作见随包 `MANUAL_ACCEPTANCE_R23.md`。这是私人测试交付包，第三方地图／模型与可公开的 GitHub 源代码分开管理。


R29 客户端增加 Oculus 1.8.0。运行 `Install-Visuals.bat` 从作者地址下载已校验的 Complementary Unbound r5.3 并启用；光影文件不捆绑。客户端可关闭光影，服务器不安装 Oculus。UN 运输、电话、核能与 UN-01 飞行见 `R29_TEST_GUIDE_CN.md`。
