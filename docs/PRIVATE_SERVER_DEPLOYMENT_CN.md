# Project SEELE R27 服务器部署

使用同一批 `Project_SEELE_R27_Server.zip`、`Project_SEELE_R27_World.zip` 和 `Project_SEELE_R27_Client.zip`，校验值在 `SHA256SUMS.txt`。两端 Project SEELE JAR 完全相同；本轮网络协议仍为 32，但请整批更新 R27 客户端和服务器以获得相同的城市控制与镜头行为；存档仍为 `SEELE_TV_WORLD_PREVIEW_20260906`。
...
3. 新建 `SEELE_TV_WORLD_PREVIEW_20260906` 子目录，将 World ZIP 内容解压进去。最终应有 `SEELE_TV_WORLD_PREVIEW_20260906/level.dat` 和 `r27_ready.json`，不要套两层同名目录。
4. 阅读 Minecraft EULA 后自行将 `eula.txt` 改为 `eula=true`。
5. 运行 `Start-Server.bat`；控制台应出现 `Project SEELE initialized` 与 `Done`。启动器在缺少 R27 存档时会停止，避免误建空世界。
...
先按 `R27_TEST_GUIDE_CN.md` 验证电梯、NPC 出动与回收、车站换乘、姿态和电源。既有 UN、载具与飞机操作见随包 `MANUAL_ACCEPTANCE_R23.md`。这是私人测试交付包，第三方地图／模型与可公开的 GitHub 源代码分开管理。
