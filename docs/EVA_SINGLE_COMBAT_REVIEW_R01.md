...
> 2026-08-27 方向修正：首版高精模型导出错误沿用了 G1 的逻辑 Bedrock
> 前向，导致 EVA 物理 `+X` 被送到模型背面 `-Z`。R03 已改为 Tiger 网格的
> Gecko-authored 契约：物理前方 `+X -> +Z`、物理左侧 `+Y -> authored +X`
>（网格渲染时 X 反射）、上方 `+Z -> +Y`。前向与左右对齐均为 `1.0`，
> 变换行列式为 `+1`。同名 `*_eva_model.mp4` 已覆盖为 R03 正面版本；旧视频
> 永久作废。
