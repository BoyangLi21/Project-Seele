@echo off
setlocal
title Project SEELE - R38 Transport recovery, connected joints and facility lighting
pushd "%~dp0"
...
for %%F in (eva_combat_capture_r31.json eva_combat_capture_r31_un00.json eva_combat_capture_r31_un01.json angel_grip_r31.json eva_recovery_r31.json eva_gameplay_r32_0.json eva_gameplay_r32_1.json eva_gameplay_r32_2.json eva_gameplay_r32_3.json eva_gameplay_r32_4.json sachiel_gameplay_r32.json articulated_bodies_r35.json) do (
    if not exist "run\projectseele-local-maps\%%F" (
        echo ERROR: Missing R38 runtime profile: %%F
        goto failed
    )
)
set "SEELE_PYTHON="
...
"%SEELE_PYTHON%" tools\check_runtime_r38.py
if errorlevel 1 goto failed
echo R38 runtime - protocol 43. Use the matching R38 client and server.
echo Guide: %CD%\docs\COMBAT_RECOVERY_LIGHTING_R38.md
...
goto done
:usage
echo Usage: start_eva_test_r38.bat [--check] [--city-shaders] [--no-shaders]
