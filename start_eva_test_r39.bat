@echo off
setlocal
title Project SEELE - R39 Transport, NPC return and brighter interiors
pushd "%~dp0"
...
for %%F in (eva_combat_capture_r31.json eva_combat_capture_r31_un00.json eva_combat_capture_r31_un01.json angel_grip_r31.json eva_recovery_r31.json eva_gameplay_r32_0.json eva_gameplay_r32_1.json eva_gameplay_r32_2.json eva_gameplay_r32_3.json eva_gameplay_r32_4.json sachiel_gameplay_r32.json articulated_bodies_r35.json) do (
    if not exist "run\projectseele-local-maps\%%F" (
        echo ERROR: Missing R39 runtime profile: %%F
        goto failed
    )
)
set "SEELE_PYTHON="
...
"%SEELE_PYTHON%" tools\check_runtime_r39.py
if errorlevel 1 goto failed
echo R39 runtime - protocol 44. Use the matching R39 client and server.
echo Guide: %CD%\docs\TRANSPORT_RETURN_R39.md
echo World: SEELE_R31_WORLD
"%SEELE_PYTHON%" tools\launch_rendered_client_r17.py --world SEELE_R31_WORLD --heap 6G --gradle-offline %SEELE_SHADERS% %SEELE_CHECK%
set "SEELE_EXIT_CODE=%ERRORLEVEL%"
if not "%SEELE_EXIT_CODE%"=="0" goto failed
goto done
:usage
echo Usage: start_eva_test_r39.bat [--check] [--city-shaders] [--no-shaders]
