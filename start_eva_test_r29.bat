@echo off
setlocal
title Project SEELE - R29 city and UN operations
pushd "%~dp0"
...
goto args
:ready
if not exist "run\saves\SEELE_TV_WORLD_PREVIEW_20260906\r29_ready.json" (
    echo ERROR: The R29 world installation is not ready.
    goto failed
)
...
echo R29 - protocol 33. Use the matching R29 client and server.
echo Guide: %CD%\docs\MANUAL_ACCEPTANCE_R29.md
echo World: SEELE_TV_WORLD_PREVIEW_20260906
"%SEELE_PYTHON%" tools\launch_rendered_client_r17.py --world SEELE_TV_WORLD_PREVIEW_20260906 --heap 6G %SEELE_SHADERS% %SEELE_CHECK%
set "SEELE_EXIT_CODE=%ERRORLEVEL%"
if not "%SEELE_EXIT_CODE%"=="0" goto failed
goto done
:usage
echo Usage: start_eva_test_r29.bat [--check] [--city-shaders] [--no-shaders]
